...
# via configupload is complete.  It uses the config values generated by
# configupload. 
# - Run these tests from the directory in which this script resides
# - Run these tests from the user account that was used to install
#   the ftp_upload software
#
...
test_ftp_upload() {
    ...
    then
        ...
        local path="$cs_ftp_dir/$testdate"
    fi

    # remove any preexisting test files on the cloud server
    #
    ssh $cs_user@$cs_name "rm -rf $path; ls -d $path > /dev/null 2>&1"
    local status=$?
    if [ "$status" -ne 2 ]
    then
        fail "Could not remove $cs_user@$cs_name:$path"
    fi
...
EndOfCommands
    ...
    #
    local nfiles=`ssh $cs_user@$cs_name "ls $path/$testcam|wc -l"`
    if [ "$nfiles" -ne 3 ]
    then
