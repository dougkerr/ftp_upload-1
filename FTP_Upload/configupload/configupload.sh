...
configure() {
    ...
    # reload the daemons that advertise our name--this list may be incomplete.
    # Should restart systemd-logind as well, but apparent bug causes Gnome to
    # crash an logout console user when its restarted
    systemctl reload avahi-daemon.service nmbd #systemd-logind.service
